<div class="flex items-start max-md:flex-col">
    <div class="fixed bottom-0 left-0 z-50 w-full md:me-10
        ...
        <flux:navlist class="flex flex-row gap-2 overflow-x-auto md:flex-col md:gap-1 items-center">
            ...
            @can('news.manage')
            <flux:navlist.item :href="route('news_management.news.index')" class="h-14" :current="request()->routeIs('news_management.news.index')" wire:navigate>
            ...
            <span class="flex flex-col items-center justify-center md:items-start md:justify-start gap-1 md:flex-row md:gap-2">
                ...
                <span class="md:inline">{{ __(key: 'Übersicht') }}</span>
                </span>
            </flux:navlist.item>

            <flux:navlist.item :href="route('news_management.areas.index')" class="h-14" :current="request()->routeIs('news_management.areas.index')" wire:navigate>
            <span class="flex flex-col items-center justify-center md:items-start md:justify-start gap-1 md:flex-row md:gap-2">
                <flux:icon.folder class="h-5 w-5 md:hidden" />
                <span class="md:inline">{{ __(key: 'Bereiche') }}</span>
                </span>
            </flux:navlist.item>
            @endcan
            </flux:navlist>
