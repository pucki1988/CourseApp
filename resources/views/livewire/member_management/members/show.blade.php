...
new class extends Component {
    ...
    public function openEditMembership(int $membershipId): void
    {
        ...
        $this->editPayerMemberId = $membership->payer_member_id;
    ...
    public function confirmExit()
    {
        ...
        $this->member->update(['left_at' => $this->exitDate]);
        
        $this->member->statusHistory()->create([
            'action' => 'exited',
            'action_date' => $this->exitDate,
        ]);
    ...
    {
        if ($this->member->deceased_at) {
            Flux::toast('Verstorbene Mitglieder können nicht reaktiviert werden', variant: 'danger');
            return;
        }
        ...
        $this->member->update(['left_at' => null]);
        $this->member->refresh();
    ...
    public function confirmDeceased()
    {
        ...
        $this->member->update([
            'deceased_at' => $this->deceasedDate,
            'left_at' => $this->deceasedDate,
        ]);
        
        $this->member->statusHistory()->create([
            'action' => 'deceased',
            'action_date' => $this->deceasedDate,
        ]);
    ...
    <x-members.layout :heading="__('Mitglied: ') . ($member->first_name . ' ' . $member->last_name)">
        ...
        <div class="border rounded-lg p-3 bg-white shadow-sm mb-4">
            <div class="flex justify-end mb-3">
                <flux:button size="sm" icon="plus" wire:click="openAssignMembership">Mitgliedschaft zuweisen</flux:button>
            </div>
    ...
    <flux:modal name="edit-membership-modal" :dismissible="false">
        <div class="space-y-6">
            ...
            <form wire:submit.prevent="updateMembership" class="space-y-4">
                ...
                <flux:select label="Zahler" wire:model.live="editPayerMemberId">
                    <option value="">Bitte wählen</option>
